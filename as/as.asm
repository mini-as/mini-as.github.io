...
alloc_memory:
	...
	mov ebx, 0
	push PAGE_READWRITE ; Constant
	push MEM_COMMIT ; Type of memory allocation
	push eax ; size of the region (bytes)
	push ebx ; 0 = system determines where to allocate
	call [VirtualAlloc]
	cmp eax, 0 ; function fails ?
	je not_enough_memory
	mov dword [memory_start], eax ; eax = base address of the allocated region of pages
	add eax, dword [memory_end]
	mov dword [memory_end], eax
	ret
...
free_memory:
	...
	je no_alloc_memory
	mov ebx, 0
	push MEM_RELEASE ; type of free operation (decommits & releases)
	push ebx ; must be 0
	push eax ; eax = pointer to the base address
	call [VirtualFree]
	no_alloc_memory:
	ret
...
asm_   db 'minis assembler version 0.09a',13,10,0
